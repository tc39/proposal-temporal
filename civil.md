...
**Con:** The word can be easilly confused with "locale" (as in i18n), two very different concepts in the ECMAScript standard.  In addition, Temporal types will have `toLocaleString` methods, which could be confusing if the type names are also called "Local".
