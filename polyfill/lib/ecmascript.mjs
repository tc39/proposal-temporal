...
export function ToTemporalMonthDay(item, options = undefined) {
  ...
  if (referenceISOYear === undefined) {
    ...
    return CreateTemporalMonthDay(month, day, 'iso8601', 1972);
  }
...
export function UnbalanceDateDurationRelative(years, months, weeks, days, plainRelativeTo) {
  ...
  const yearsMonthsWeeksInDays = DaysUntil(isoDate, later);
...
export function AddDurationToOrSubtractDurationFromPlainYearMonth(operation, yearMonth, durationLike, options) {
  ...
  const addedPlainDate = CreateTemporalDate(addedDate.year, addedDate.month, addedDate.day, calendar);
  const addedDateFields = PrepareTemporalFields(addedPlainDate, fieldNames, []);
...
export function DaysUntil(earlier, later) {
  return DifferenceISODate(earlier.year, earlier.month, earlier.day, later.year, later.month, later.day, 'day').days;
}
