...
export function GetTemporalRelativeToOption(options) {
  ...
  if (Type(relativeTo) === 'Object') {
    ...
    let tzAnnotation, z, year, month, day, time;
    ...
    if (time === 'start-of-day') {
      if (timeZone !== undefined) {
        // start-of-day signifies that we had a string such as YYYY-MM-DD[Zone].
        // It is grammatically not possible to specify a UTC offset in that
        // string, so the behaviour collapses into ~WALL~, which is equivalent
        // to offset: "ignore".
        assert(offsetBehaviour === 'wall', 'offset cannot be provided in YYYY-MM-DD[Zone] string');
        assert(offset === undefined, 'offset cannot be provided in YYYY-MM-DD[Zone] string');
...
export function InterpretISODateTimeOffset(
  ...
  if (offsetBehaviour === 'exact' || offsetOpt === 'use') {
    ...
    const epochNs = GetUTCEpochNanoseconds(isoDateTime).subtract(offsetNs);
...
export function ToTemporalZonedDateTime(item, options = undefined) {
  ...
  if (Type(item) === 'Object') {
    ...
    if (time === 'start-of-day') {
      // start-of-day signifies that we had a string such as YYYY-MM-DD[Zone].
      // It is grammatically not possible to specify a UTC offset in that
      // string, so the behaviour collapses into ~WALL~, which is equivalent to
      // offset: "ignore".
      assert(offsetBehaviour === 'wall', 'offset cannot be provided in YYYY-MM-DD[Zone] string');
      assert(offset === undefined, 'offset cannot be provided in YYYY-MM-DD[Zone] string');
      const epochNanoseconds = GetStartOfDay(timeZone, isoDate);
...
export function GetISODateTimeFor(timeZone, epochNs) {
  ...
  const { isoDate, time } = GetISOPartsFromEpoch(epochNs);
  const { hour, minute, second, millisecond, microsecond, nanosecond } = time;
  const balancedTime = BalanceTime(hour, minute, second, millisecond, microsecond, nanosecond + offsetNs);
  const balancedDate = AddDaysToISODate(isoDate, balancedTime.deltaDays);
