...
// Years outside the range 1900-2100 may be approximated
export const abbreviatedInterestingYears = interestingYears.filter((y) => y >= 1900 && y <= 2100);
...
for (const year of abbreviatedInterestingYears) {
  for (const [month, day] of interestingMonthDays) {
    ...
    const dateChinese = date.withCalendar('chinese');

    // Pre-compute toString so it's not done repeatedly in each test
    interestingCases.push([dateChinese, date.toString()]);
  }
}
...
await time(async (start) => {
  ...
  await withSnapshotsFromFile('./chinese.snapshot.json', (matchSnapshot) => {
