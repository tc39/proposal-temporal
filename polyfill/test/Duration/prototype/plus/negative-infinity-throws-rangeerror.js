...
/*---
description: Temporal.Duration.prototype.plus throws a RangeError if any value in a property bag is -Infinity
...
overflows.forEach((overflow) => {
  fields.forEach((field) => {
    assert.throws(RangeError, () => instance.plus({ [field]: -Infinity }, { overflow }));
  });
});
...
overflows.forEach((overflow) => {
  fields.forEach((field) => {
    calls = 0;
    assert.throws(RangeError, () => instance.plus({ [field]: obj }, { overflow }));
