...
class MyTime extends Temporal.PlainTime {
  constructor(hour, minute, second, millisecond, microsecond, nanosecond, calendarArg) {
    assert.compareArray([hour, minute, second, millisecond, microsecond, nanosecond], [12, 34, 56, 987, 654, 321], "constructor arguments");
    if (called === 1) {
      assert.sameValue(calendarArg, calendar, "calendar argument");
    }
    ++called;
    super(hour, minute, second, millisecond, microsecond, nanosecond, calendarArg);
  }
}
