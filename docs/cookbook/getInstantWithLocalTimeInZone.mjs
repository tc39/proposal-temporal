...
function getInstantWithLocalTimeInZone(dateTime, timeZone, disambiguation = 'earlier') {
  ...
  const zdts = ['earlier', 'later']
    .map(disambiguation => dateTime.toZonedDateTime(timeZone, { disambiguation }));
  const instants = zdts
  ...
  switch (disambiguation) {
    case 'clipEarlier':
      ...
      return zdts[0].nextTransition().add({ nanoseconds: -1 }).toInstant();
    case 'clipLater':
      if (zdts[1].toPlainDateTime().equals(dateTime)) {
        return instants[1];
      }
      return zdts[0].nextTransition().toInstant();
  }
