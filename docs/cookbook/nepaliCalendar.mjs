 ...
 *
 * The data starts from 1970 (1913 in ISO calendar) to 2100 (2044 in ISO)
 */
...
class NepaliPlainDate extends Temporal.PlainDate {
  ...
  // dayOfWeek and daysInWeek inherited from Temporal.PlainDate directly. Nepali
  // calendars seem to use ISO days of the week, with localized names.
  get dayOfYear() {
  ...
  equals(other) {
    return other instanceof NepaliPlainDate && super.equals(other);
  }

  toString({ showCalendar = 'auto', ...options } = {}) {
    let result = super.toString({ ...options, showCalendar: 'never' });
  ...
  toJSON() {
    return super.toString({ showCalendar: 'never' }) + '[u-ca=nepali]';
  }

  // Note: for brevity, deserialization from a string is not implemented.
  static from(fields, { overflow = 'constrain' } = {}) {
    void overflow; // TODO: use overflow?
    const iso = nepaliToISO(fields);
  ...
  #isoToNepali() {
    const isoDayOfYear = super.dayOfYear;
    let nepaliYear = super.year + 56;
  ...
  with(dateLike, { overflow = 'constrain' } = {}) {
    void super.with(dateLike, { overflow });
  ...
  add(duration, { overflow = 'constrain' } = {}) {
    void super.add(duration, { overflow });
    throw new Error('not implemented');
  }
  subtract(duration, { overflow = 'constrain' } = {}) {
    void super.subtract(duration, { overflow });
    throw new Error('not implemented');
  }
  until(other, { largestUnit = 'day' } = {}) {
    void super.until(other, { largestUnit });
    throw new Error('not implemented');
  }
  since(other, { largestUnit = 'day' } = {}) {
    void super.since(other, { largestUnit });
    throw new Error('not implemented');
  }
}
...
function daysInISOYear(year) {
  const isDiv4 = year % 4 === 0;
  const isDiv100 = year % 100 === 0;
  const isDiv400 = year % 400 === 0;
  return isDiv4 && (!isDiv100 || isDiv400) ? 366 : 365;
}
...
function getNepaliYearData(year) {
  if (year < firstSupportedNepaliYear || year > lastSupportedNepaliYear) {
    throw new Error(
...
// For brevity, conversion from year-monthCode-day is not implemented
function nepaliToISO({ year: nepaliYear, month: nepaliMonth, day: nepaliDay = 1 }) {
  let yearData = getNepaliYearData(nepaliYear);
  ...
  if (nepaliMonth === 9) {
    ...
    if (isoDayOfYear < 0) {
      isoDayOfYear += daysInISOYear(isoYear);
    }
  } else {
