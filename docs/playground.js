(function (global, factory) {
	...
	} = ObjectGetOwnPropertyDescriptor((IntlDateTimeFormat === null || IntlDateTimeFormat === void 0 ? void 0 : IntlDateTimeFormat.prototype) || ObjectCreate(null), 'format') || ObjectCreate(null);
	const {
	...
	} = (IntlDateTimeFormat === null || IntlDateTimeFormat === void 0 ? void 0 : IntlDateTimeFormat.prototype) || ObjectCreate(null);
	...
	function AdjustDateDurationRecord(_ref7, newDays, newWeeks, newMonths) {
	  let {
	    years,
	    months,
	    weeks
	...
	function ISODateToString(_ref8) {
	  let {
	    year,
	    month,
	    day
	...
	function TimeRecordToString(_ref9, precision) {
	  let {
	    hour,
	    minute,
	    second,
	...
	function RejectYearMonthRange(_ref0) {
	  let {
	    year,
	    month
	  } = _ref0;
	...
	function CheckISODaysRange(_ref1) {
	  let {
	    year,
	    month,
	    day
	...
	function RoundRelativeDuration(duration, originEpochNs, destEpochNs, isoDateTime, timeZone, calendar, largestUnit, increment, smallestUnit, roundingMode) {
	  ...
	  if (irregularLengthUnit) {
	    ...
	    ({
	      nudgeResult
	    } = NudgeToCalendarUnit(sign, duration, originEpochNs, destEpochNs, isoDateTime, timeZone, calendar, increment, smallestUnit, roundingMode));
	  } else if (timeZone) {
	...
	function AddTime(_ref10, timeDuration) {
	  let {
	    hour,
	    minute,
	    second,
	...
	impl['iso8601'] = {
	  ...
	  monthDayToISOReferenceDate(fields, overflow) {
	    ...
	    const {
	      month,
	      day
	    } = RegulateISODate((_fields$year = fields.year) !== null && _fields$year !== void 0 ? _fields$year : referenceISOYear, fields.month, fields.day, overflow);
	    return {
	      month,
	      day,
	      year: referenceISOYear
	...
	const nonIsoHelperBase = {
	  ...
	  completeEraYear(calendarDate) {
	    ...
	    if (year !== undefined) {
	      ...
	      ({
	        eraYear,
	        era
	      } = this.eraFromYear(adjustedCalendarDate));
	    }
	  ...
	  adjustCalendarDate(calendarDate, cache, overflow /*, fromLegacyDate = false */) {
	    ...
	    ({
	      month,
	      monthCode
	    } = resolveNonLunisolarMonth(calendarDate, this.id, overflow));
	      ...
	      if (sign) {
	        ...
	        if (diff.years === 0 && diff.months === 0) {
	          ...
	          if (increment > 1) {
	            ...
	            // rejected here.
	            if (overflow === 'reject') {
	              throw new RangeError$1("Can't find ISO date from calendar date: ".concat(JSONStringify(_objectSpread2({}, originalDate))));
	            } else {
	  ...
	  monthDayFromFields(fields, overflow, cache) {
	    ...
	    [monthCode, maxDayForMonthCode] = this.maxLengthOfAdjustedMonthCodeInAnyYear(monthCode, day, overflow);
	...
	const helperHebrew = makeNonISOHelper([{
	  ...
	  adjustCalendarDate(calendarDate, cache) {
	    ...
	    let {
	      ...
	      day,
	      monthExtra
	    } = this.completeEraYear(calendarDate);
	    if (fromLegacyDate) {
	...
	const helperChinese = ObjectAssign({}, nonIsoHelperBase, {
	  ...
	  maxLengthOfAdjustedMonthCodeInAnyYear(monthCode, day, overflow) {
	    let [refYear29, refYear30] = this.monthDayISOReferenceYears[monthCode];
